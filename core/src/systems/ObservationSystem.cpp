...
void addEnvironmentalClues(
    ...
    if (state.basilisk.alive && state.world.areConnected(viewer.cave, state.basilisk.cave)) {
        observations.push_back(PlayerObservation{
            state.basilisk.behavior == BasiliskBehavior::Lurker
                ? ObservationType::BasiliskNearbySubtle
                : ObservationType::BasiliskNearby,
            viewer.id
        });
    }

    if (state.basilisk.alive && state.basilisk.behavior == BasiliskBehavior::Restless) {
        const bool movedThisRound = std::any_of(events.begin(), events.end(),
            [](const GameEvent& event) {
                return event.type == GameEventType::BasiliskMoved &&
                       event.basiliskBehavior == BasiliskBehavior::Restless;
            });
    ...
    if (state.basilisk.alive &&
        ...
        state.basilisk.lastCave.has_value()) {
        observations.push_back(PlayerObservation{
            ObservationType::EnragedLastKnownCave,
            viewer.id,
            state.basilisk.lastCave
        });
    }
}

void addEventFeedback(
    const MatchState& state,
    const PlayerState& viewer,
    const std::vector<GameEvent>& events,
    std::vector<PlayerObservation>& observations) {
    ...
    for (const auto& event : events) {
        if (event.type == GameEventType::ArrowReachedBasilisk && event.actor == viewer.id) {
            viewerReachedBasilisk = true;
        }
        if (event.type == GameEventType::BasiliskKilled && event.actor == viewer.id) {
            viewerKilledBasilisk = true;
        }
        ...
        switch (event.type) {
            ...
            case GameEventType::ItemFound:
                if (event.actor == viewer.id) {
                    PlayerObservation observation{ObservationType::ItemFound, viewer.id};
                    observation.cave = event.cave;
                    observation.itemType = event.itemType;
...
std::vector<PlayerObservation> ObservationSystem::buildForPlayer(
    const MatchState& state,
    PlayerId viewerId,
    const std::vector<GameEvent>& events) {
