...
void DebugInventoryMenuState::moveSelection(int direction) noexcept {
    ...
    const int selected = static_cast<int>(selected_);
    selected_ = static_cast<std::size_t>((selected + direction + count) % count);
}
...
bool renderDebugInventoryMenu(
    ...
    const float height = (92.0F + 42.0F * kItems.size()) * scale;
