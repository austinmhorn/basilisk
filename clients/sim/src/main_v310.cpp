...
// V3.10 intentionally reuses the proven V3.9 simulator implementation in one
// translation unit, then replaces only the match driver/report entrypoint with
// additional Enraged telemetry. This is a single-level instrumentation shim;
// main_v39.cpp does not include any older simulator source.
    ...
    for (const auto& event : events) {
        switch (event.type) {
            ...
            case GameEventType::PlayerKilled:
                ...
                // Event-order attribution, with the most direct cause first.
...
void finalizeEnragedMatch(
    ...
    if (tracker.huntersAliveAtAssignment >= 2) {
        ...
        // This is the cleanest measure of what the surviving rival does after
        // Enraged takes exactly one of two hunters.
        if (tracker.contactKills == 1) {
...
void printV310Report(
    ...
    // Reuse the complete V3.9 cumulative report, replacing only its version
    // label, then append the V3.10-only science block.
...
int main(int argc, char** argv) {
    ...
    for (std::uint64_t i = 0; i < matches; ++i) {
        runOneV310(firstMapSeed + static_cast<MapSeed>(i),
            firstMatchSeed + static_cast<MatchSeed>(i), maxRounds, stats, enragedTelemetry);
    }

    printV310Report(stats, enragedTelemetry, maxRounds);
    return 0;
}
